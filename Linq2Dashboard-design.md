...
8. **Templates and styling last**, once the defaults work (§C7): optional `ValueTemplate` and `HeaderTemplate` per facet component, then the stylesheet. **Done.**
   - Templates: `HeaderTemplate` on `ValueFacet`, `RangeFacet` and `DateFacet` receives the facet state and replaces the default header; `ValueTemplate` on `ValueFacet` receives the `FacetValue` and replaces the label and count inside the button, so the click stays; `MetricTemplate` on `Metric` receives a `MetricTileContent` with the formatted pieces and replaces the whole tile, wrapper included.
