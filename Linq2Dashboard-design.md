...
11. **Markup definitions (added 2026-09-26).** Under a view with `Items`, the facet and metric components define what they show (§C7), on the same components as display rather than separate definition components. Done for every facet component and `Metric`. `RangeFacet` defines from `For` with `Buckets` or `AutoBuckets` (both throw), `DateFacet` with `TimeZone` (watched by id), `Granularity` or `AutoGranularity` (both throw), `Presets` and `SkipEmptyPresets`, each with a `Define` over its builder; their value type is read from the selector as for `ValueFacet`, and a selector the kind cannot take fails with the builder's own message. `TextFacet` has no selector, so its `Match` function is what defines it (always explicit); `Define` without `Match` throws, and the component reads the facet's text only once the facet exists. `Metric` defines from exactly one of `Count`, `Sum`, `Average`, `Min`, `Max`, `Distinct` (boxed selectors, unboxed and typed like `For`) and `Formula`, with a `Define` over `MetricBuilder<T>`; two aggregations, or `Define` without one, throw. The aggregation kind, the selector's type and `Name` are watched. A facet and a metric may share a key, since they are separate key spaces in the core. Plain metrics are defined before formulas, so a formula can read a plain metric declared anywhere in the markup; a formula whose input is still undefined gets the core's message plus a hint naming the likely causes, a later formula or a part of the page that has not rendered. Blazor keeps a parameter's last value when it disappears from the markup, so switching `Sum` to `Average` means setting the unused one to null.
...
- **The view cascades its type parameter, keeping the name `T`.** Children written inside `DashboardView` need no `T="Order"`, and the view infers its own from `Dashboard` or `Items`, so a page names the row type nowhere; explicit `T` still compiles. `T` is kept over a rename to `TItem`, which would have avoided sharing the name with other libraries' components but broken every page: the compiler lets another component's own parameters win, so the only effect is on a component whose `T` nothing infers. A method group for a parameter typed by `T` needs `T` on that component, since the compiler reads it as inferring; both are in the usage guide. Decided 2026-09-26. See §9, component 1.
