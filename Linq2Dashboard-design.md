...
11. **Markup definitions (added 2026-09-26).** Under a view with `Items`, the facet and metric components define what they show (§C7), on the same components as display rather than separate definition components. Done for `ValueFacet`; `RangeFacet`, `DateFacet`, `TextFacet` and `Metric` follow the same pattern.
